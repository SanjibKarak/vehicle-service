...
public class VehicleService {
	...
	public VehicleEntity getVehicleByType(String vehicleType) {
		VehicleEntity v = null;
		try {
			v = this.vehicleRepository.findByVehicleType(vehicleType);
		} catch (Exception e) {
			e.printStackTrace();
		}
		// v = list.stream().filter(e -> e.getVehicleType() ==
		// vehicleType).findFirst().get();
		return v;
